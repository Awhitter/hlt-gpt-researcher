...
- Do not substitute a consumer data shape for an ownership answer. When the user asks both what data exists and who owns it, answer ownership separately for each field or workflow that the opened evidence proves. For everything else, say ownership was not verified; only then mention an observed consumer shape if it is useful.
- Capture timing and placement must come from opened mount or call sites. Do not invent illustrative occasions (for example newsletter signup or job-application start) merely because a reusable email component could be mounted there.
...
- A present-tense code path is not live-runtime proof. For questions containing "currently" or asking what is live, describe repository evidence as an implemented path or capability unless an opened runtime readback or operational receipt proves that it is configured and executing. Apply this especially to outbound systems such as Marketo.
...
- Rewrite unqualified operational claims such as "emails are sent to Marketo" as implemented capability plus an explicit live-runtime boundary unless Context contains live readback proving present execution.
- Do not preserve illustrative capture occasions from the draft. Each claimed occasion must have an opened mount or call site in Context; otherwise state only the exact component submission behavior that was verified.
