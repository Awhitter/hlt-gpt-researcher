...
The Slack face of Mastery Research. Runs the NousResearch Hermes runtime with
MCP mounts to GPT Researcher, codegraph, Katailyst2 and Linear, with memory on a
Render disk.

**"Hermes" is the runtime, not the agent.** The agent is Brian. `agent:hermes`
in Katailyst2 is the fleet orchestrator persona — a different thing — so never
call this Hermes in docs, Slack, or the registry. The Render service keeps the
...
1. installs `SOUL.md` into `$HERMES_HOME` ([`grounding.py`](./grounding.py)),
2. renders `$HERMES_HOME/config.yaml` from the environment
   ([`render_config.py`](./render_config.py)),
3. supervises `hermes gateway` as a child when `BRIAN_ENABLE_GATEWAY=1`,
...
who can @mention Brian gets arbitrary code execution on this container.

Brian also reads untrusted third-party web pages. Shell access plus untrusted
...
`tests/test_brian_boot.py::test_slack_toolset_excludes_host_access` fails the
...
## Where Brian's knowledge lives

| Slot | Content | Why there |
|---|---|---|
| [`grounding/SOUL.md`](./grounding/SOUL.md) | identity, voice, what he won't do | loaded from `$HERMES_HOME` every session |
| [`grounding/AGENTS.md`](./grounding/AGENTS.md) | the durable company briefing | read from `terminal.cwd`; ships read-only in the image so Brian can't rewrite his own facts |
...
| `BRIAN_ENABLE_GATEWAY` | `1` starts the Slack gateway; anything else = health only |
...
## Turning Brian on (one-time, ~5 minutes)

Steps 1–3 need Alec's login; they create a Slack app and mint tokens.
...
| `gateway` | Brian is running. `gateway.uptime_seconds` climbs. |
...
is empty. Then DM Brian "what can ScraperVault do?" and confirm the reply cites
codegraph.

**Adversarial check, worth doing once:** from a non-admin account, confirm
`/model` is refused, and ask Brian to run a shell command — he should say he
